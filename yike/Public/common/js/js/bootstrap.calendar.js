...
(function($, window, document, undefined) {
	...
	var pluginName = 'Calendar',
		...
		template = '' + '<div class="calendar" id="calendar">' + '<div class="calendar-header"></div>' + '<div class="calendar-body"></div>' + '</div>' + '',
	...
	Plugin.prototype.init = function() {
		...
		// tap changed to click
		this.calendar = $(template.replace("%msg_today%", this.msg_today)).appendTo(this.element).click($.proxy(this.click, this));
	...
	Plugin.prototype.renderCalendar = function(date) {
		...
		for (var i = 0; i >= last; i++) {
			...
			for (var j = this.weekStart; j < this.days.length + this.weekStart; j++) {
				...
				// Render HTML
				if (dow == 0) {
					_html += '<div class="calendar-td ' + cls + ' nothing' + '" >&nbsp;</div>';
	...
	Plugin.prototype.renderDays = function() {
		...
		for (var j = this.weekStart; j < this.weekStart + 7; j++) {
			var thcls;
			if (j % 7 == 0 || j % 7 == 6) {
				thcls = "calendar-th weekend";
			} else {
				thcls = "calendar-th";
			}
			html += "<div class='" + thcls + "'>" + this.days[j % 7] + "</div>";
		}
