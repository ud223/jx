...
class Payment {
  ...
  public function WeChatShopPayParam($_order, $openId){
    ...
    //设置商品或支付单简要描述
    $input->SetBody("高尔夫训练场约课");//("{$this->SiteConfig->SiteName}-订单编号{$_order["order_sn"]}");
    ...
    $input->SetTotal_fee(1);//($_order["order_amount"] * 100);
    ...
    //设置接收微信支付异步通知回调地址
    $input->SetNotify_url("http://www.gaoshougolf.com/show/pay_notify");
  ...
  public function WeChatShopGroupBuyPayParam($_order, $openId){
    ...
    //设置商品或支付单简要描述
    $input->SetBody("高手高尔夫"); //("{$this->SiteConfig->SiteName}-订单编号{$_order["order_sn"]}");
