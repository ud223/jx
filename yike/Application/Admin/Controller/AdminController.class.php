...
class AdminController extends Controller {
    ...
    protected function _initialize(){
        ...
        //echo session('user_type');exit;
        $user_type = session('user_type');
        $menu = D('menu');
        //判断当前菜单是否可用
        $condition['enable'] = array('eq',1);

        $m_menu = $menu->where($condition)->order('sort')->select();
...
//        $m_menu_info = $this->parseMenu($m_menu);

//        $this->assign('menu',$m_menu_info);

        $this->assign('model', $model_menu);
        $this->assign('fun', $fun_menu);
...
//                }
                ...
                if (empty($menu_arr[$itm['pid']]['lv1'])) {
                    $menu_arr[$itm['pid']]['lv1'] = array();
                }

                $menu_arr[$itm['pid']]['lv1'][] = $itm;

//                array_push($menu_arr[$itm['pid']]['lv1'],$itm);
...
//        print_r($menu_arr);exit;
    ...
    final public function getMenus($controller=CONTROLLER_NAME){
        ...
        if(empty($menus)){
            ...
            $menus['main']  =   M('Menu')->where($where)->order($sort)->field('id,title,url')->select();
