...
class ComController extends BaseController
{
    ...
    public function _initialize()
    {
        ...
        if ($userinfo[0]['group_id'] != 1 && $userinfo[0]['group_id'] != 6 && !$Auth->check(CONTROLLER_NAME . '/' . ACTION_NAME,
...
//        $current_action_name = ACTION_NAME == 'edit' ?"index" : ACTION_NAME;
    ...
    protected function cateTreelist($pid = 0, $level = 0) {
        $cate = M('category');
        $array = array();
        ...
        if (is_array($tmp)) {
            foreach ($tmp as $v) {
                $v['level'] = $level;
                $array[count($array)] = $v;
                $sub = $this->cateTreelist($v['id'], $level + 1);
                if (is_array($sub))
